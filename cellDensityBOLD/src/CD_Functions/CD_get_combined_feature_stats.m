function tbl = CD_get_combined_feature_stats(data, single_stats, combined_stats, new_table)
% GET_COMBINED_FEATURE_STATS Generate a table of feature value summary statistics using multiple datasets
%     Similarly to 'CD_get_feature'stats', this function accepts some data in the 
%     form of a struct and returns a table of queried summary statistics. 
...
%                         individually; see 'CD_get_feature_stats'. Can be left empty, 
...
%         tbl:            A table of operation information and summary statistics
    ...
    for i = 1:length(table_container)
        table_container{i} = CD_get_feature_stats(data(i, :), single_stats, []);
    end
        ...
        try
            switch the_stat{1}
                ...
                case 'Feature_Value_Gradient_RMSE'
                    ...
                    x = arrayfun(@(x) x.Inputs.subject, data)';
                ...
                case 'Feature_Value_Gradient_Absolute_Correlation'
                    ...
                    the_stat_values = abs(corr(arrayfun(@(x) x.Inputs.subject, data), original_tbl{:, what_columns}')');
                    if sum(what_columns) == 0
                        error("'%s' requires the single_stat 'Feature_Value_Gradient'", the_stat{1})
                    end
                ...
                case 'Average_RMSE_Constant_Gradient'
                    ...
                    what_columns = contains(original_tbl.Properties.VariableNames, 'Feature_Value_Intercept');
                    b = tbl{:, what_columns}';
                    subjectind = arrayfun(@(x) x.Inputs.subject, data);
                    m_b = corr(b, subjectind, 'Type', 'Pearson').*std(b, [], 1)'./std(subjectind);
                    b_b = mean(b, 1)' - m_b.*mean(subjectind);
                    % m_b and b_b have features down their rows
                    % Get the average gradient
                    what_columns = contains(original_tbl.Properties.VariableNames, 'Feature_Value_Gradient');
                    m = mean(tbl{:, what_columns}, 2); % Maybe use median instead of mean, for the features that spike dramatically near 0 subjectind?
                    % Calculate intercept estimates from intercept trend
                    b = m_b.*subjectind' + b_b;
                    RMSE = zeros(size(b));
                    % Only use densities that are in the
                    % correlation range: Assumes the correlation range is
                    % the same for all subject (might have to fix for
                    % subcritical)
                    idxs = (data(1).Inputs.cp_range >= data(1).Correlation_Range(1) & data(1).Inputs.cp_range <= data(1).Correlation_Range(2))';
                    % Calculate RMSE (of predicting DENSITY from
                    ...
                    % Average these RMSE's for each feature (over the subjects)
                ...
                case 'Average_RMSE_Constant_Intercept'
                    ...
                    m = tbl{:, what_columns}';
                    subjectind = arrayfun(@(x) x.Inputs.subject, data);
                    m_m = corr(m, subjectind, 'Type', 'Pearson').*std(m, [], 1)'./std(subjectind);
                    b_m = mean(m, 1)' - m_m.*mean(subjectind);
                    % m_m (gradients of gradient) and b_m (intercepts of gradient) have features down their rows
                    % Get the average intercept
                    what_columns = contains(original_tbl.Properties.VariableNames, 'Feature_Value_Intercept');
                    b = mean(tbl{:, what_columns}, 2); % Maybe use median instead of mean
                    % Calculate gradient estimates from intercept trend
                    m = m_m.*subjectind' + b_m;
                    RMSE = zeros(size(m));
                    % Only use densities that are in the
                    % correlation range: Assumes the correlation range is
                    % the same for all subject (might have to fix for
                ...
                case 'Mean_Density_RMSE'
                    what_columns = contains(original_tbl.Properties.VariableNames, 'Density_RMSE');
                    the_stat_values = mean(tbl{:, what_columns}, 2);
                    if sum(what_columns) == 0
                        error("'%s' requires the single_stat'Density_RMSE'", the_stat{1})
                    end
