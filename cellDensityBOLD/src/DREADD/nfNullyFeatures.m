function null = nfNullyFeatures(dataDREADD, dataDensity, ops, classKeys, normalisationTrain, nReps)
%NFNULL Compute a null distribution for DREADD-Density data by randomising 
% nf directions AND empirical densities AND features.
% As expected, this gives similar distirbutions to nfNullyDensity
    ...
    for N = 1:nReps
        ...
        TS_DataMat = rand(size(TS_DataMat)); % But.. but this will ruin the scale... no, it's fine as long as we used nfNull with test normalisation on
