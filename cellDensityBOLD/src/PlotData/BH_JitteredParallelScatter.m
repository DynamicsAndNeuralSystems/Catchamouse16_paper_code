function [ff,xx] = BH_JitteredParallelScatter(dataCell,addMeans,customLine,makeFigure,extraParams)
% Modified version of BF_JitteredParallellScatter
%
% ------------------------------------------------------------------------------
...
if nargin < 3
    customLine = false; % No custom line
end
if nargin < 4
...
if ~isfield(extraParams,'theColors')
    if numGroups <= 3
        ...
        theColors = BF_GetColorMap('spectral',numGroups,1);
    end
...
for i = 1:numGroups
    ...
    % Keep for customLineing the jittered scatter points:
    xx{i} = x; ff{i} = f;
end
...
for i = 1:numGroups
    ...
    if customLine ~= false
        plot([customOffset + i - offsetRange/2,customOffset + i + offsetRange/2],customLine(i)*ones(2,1),'-',...
