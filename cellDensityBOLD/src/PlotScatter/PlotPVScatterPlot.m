...
if labelCortex
    ...
    % areaColors = BF_GetColorMap('pastel2',max(labelInd),1);
    areaColors = areaGiveMeColors('areas');
...
%-------------------------------------------------------------------------------
% Cosmetics:
xlabel(strrep(microName, '_', '\_'),'interpreter','Tex')
ylabel(propertyName,'interpreter','none')

%-------------------------------------------------------------------------------
% Add title:
isGood = ~isnan(regionProperties);
[rho,pVal] = corr(microProperties(isGood),regionProperties(isGood),'type',whatCorr,'rows','pairwise');
title(sprintf('[%u] %s''s \\rho = %.3f (p = %.3g)',sum(isGood),whatCorr,rho,pVal), 'interpreter', 'tex')

fprintf(1,'%s-%s: rho = %.2f, p = %.6g\n',microName,propertyName,rho,pVal);
